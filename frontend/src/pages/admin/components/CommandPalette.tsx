    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed top-[15%] left-1/2 -translate-x-1/2 w-full max-w-[560px] z-[950]"
            initial={{ opacity: 0, y: -20, scale: 0.96 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: -20, scale: 0.96 }}
            transition={{ duration: 0.15, ease: "easeOut" }}
          >
            <div className="bg-[var(--color-background-secondary)] rounded-2xl border border-[var(--color-border-light)] shadow-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-[var(--color-border-light)]">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent outline-none text-[var(--color-text-primary)] placeholder-[var(--color-text-tertiary)]"
                />
                ...
                {filtered.length > 0 ? (
                  filtered.map((result) => (
                    <button
                      ...
                      className="w-full flex items-center gap-4 px-5 py-3 hover:bg-[var(--color-background-tertiary)] transition-colors text-left group cursor-pointer border-b border-[var(--color-border-light)] last:border-0"
                    >
                      <div
                      ...
                      >
                        {result.type === "course" ? <BookOpen size={16} /> : <Users size={16} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-[var(--color-text-primary)]">{result.label}</p>
                        <p className="text-xs text-[var(--color-text-tertiary)] truncate">{result.description}</p>
                      </div>
                      <ArrowRight size={14} className="text-[var(--color-text-tertiary)] opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                    </button>
                  ))
                ) : (
              ...
              <div className="px-5 py-3 border-t border-[var(--color-border-light)] bg-[var(--color-background-secondary)] flex items-center gap-4 text-[10px] text-[var(--color-text-tertiary)]">
