...
export default function SavedCoursesPage() {
  ...
  const filtered = ageFilter === "All"
    ? saved
    : saved.filter(c => courseMatchesAgeGroup(c, normalizeAgeGroupId(ageFilter)));

  const stats = useMemo(() => [
    { label: "Saved", value: saved.length, icon: Bookmark, color: "var(--color-accent)" },
    { label: "Sproutlings", value: saved.filter(c => courseMatchesAgeGroup(c, "Sproutlings (5-7 age)")).length, icon: Sprout, color: "#58cc02" },
    { label: "Saplings", value: saved.filter(c => courseMatchesAgeGroup(c, "Saplings (7-14 age)")).length, icon: Sprout, color: "#14b8a6" },
    { label: "Dreamers", value: saved.filter(c => courseMatchesAgeGroup(c, "Dreamers (18+ age)")).length, icon: Sparkles, color: "#f59e0b" },
  ], [saved]);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm max-w-xl" style={{ color: "var(--color-text-secondary)" }}>
              Courses you've bookmarked for later — pick up where your curiosity left off.
            </p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--color-border-light)] bg-[var(--color-background)] shadow-sm">
              ...
              <span className="text-sm font-bold" style={{ color: "var(--color-text-primary)" }}>{saved.length} Saved</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-lg font-bold" style={{ color: "var(--color-text-primary)" }}>
                You've saved {saved.length} course{saved.length > 1 ? "s" : ""}!
              </p>
              <p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
                {saved.filter(c => c.progress && c.progress > 0).length > 0
                  ? `${saved.filter(c => c.progress && c.progress > 0).length} of them have progress — pick up where you left off.`
                  : "Ready to start learning? Pick a course and dive in."
                }
              </p>
            </div>
          </div>
